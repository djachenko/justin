...
class TitleInput:
    ...
    def set_value(self, value: str, driver: WebDriver, wait: WebDriverWait) -> None:
        el = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, f'[data-testid="{self.test_id}"]')))
...
class OptionalDateSpingroup:
    def set_value(self, value: datetime | None, driver: WebDriver, wait: WebDriverWait) -> None:
        if value is None:
            return
        end_date_btn = wait.until(EC.presence_of_element_located(
...
class EventStep1Schema:
    ...
    def submit(self, driver: WebDriver, wait: WebDriverWait) -> None:
        btn = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '[data-testid="done_button"]')))
        btn.click()
        time.sleep(0.5)
...
class EventStep3Schema:
    def submit_and_get_id(self, driver: WebDriver, wait: WebDriverWait) -> int:
        btn = wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, '[data-testid="go_to_community"]')))
        driver.execute_script("arguments[0].click()", btn)
        driver.switch_to.default_content()
        wait.until(lambda d: re.search(r'(?:event|club)\d+', d.current_url))
        return int(re.search(r'(?:event|club)(\d+)', driver.current_url).group(1))
...
class EventCreationSchema:
    ...
    def __call__(self, settings: EventCreationSettings, driver: WebDriver, wait: WebDriverWait) -> int:
        ...
        iframe = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, self._IFRAME_CSS)))
        ...
        return self.step3.submit_and_get_id(driver, wait)
